/* hull 3D
 ...
 We note that the convex hull we find does not triangulate the faces of co-planear
 points. All possible point triplets that are coplanar are returned in the hull.
 A possible solution is to allow faces to made up of more than 3 points, 
 run a 2D convex hull algorithm on the set of coplanar points and 
 then return the points in the 2D hull. To return triangulated faces 
 of coplanar points, one could keep track of faces and only add a point to 
 2 existing points on a face if the two faces don’t overlap. 
...
//vector for storing a set of random colors
vector<double> randColorVect;
//string to keep track if the color should be random or not
string color;
...
/*Initializes diamond treasure from Phillip and Jessica*/
void beautiful_diamond() {
...
/*Initializes points in a pyramid*/
void initialize_points_cross(){
...
// *Initializes points in a pyramid by Grace and Eric*/
void initialize_points_pyramid(){
	//clear the vector just to be safe
	points.clear();
...
//save three doubles at a time, as the values for the randomcolors
//and set variable for color = random
void initializeRandomColors() {
	color = "random";
	randColorVect.clear();
	for (int i = 0; i < hull.size(); i++) {
		randColorVect.push_back(((double)rand())/RAND_MAX);
		randColorVect.push_back(((double)rand())/RAND_MAX);
		randColorVect.push_back(((double)rand())/RAND_MAX);
	}
}
...
void draw_hull(){
	if (hull.size() > 0) {
		...
		for (i=0; i < hull.size()-1; i++) {
			//extract the three points from the triangle saved in hull
			point3d a = hull.at(i).a;
			point3d b = hull.at(i).b;
			point3d c = hull.at(i).c;
			
			//map the x,y,z values from a,b,c to the window
			GLfloat ax = windowtoscreen((GLfloat)a.x);
			GLfloat ay = windowtoscreen((GLfloat)a.y);
			GLfloat az = windowtoscreen((GLfloat)a.z);
			GLfloat bx = windowtoscreen((GLfloat)b.x);
			GLfloat by = windowtoscreen((GLfloat)b.y);
			GLfloat bz = windowtoscreen((GLfloat)b.z);
			GLfloat cx = windowtoscreen((GLfloat)c.x);
			GLfloat cy = windowtoscreen((GLfloat)c.y);
			GLfloat cz = windowtoscreen((GLfloat)c.z);
			
			glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
			//check what color mode is on
			if (color == "yellow") {
				glColor3fv(yellow);
			} else {
				//if random, grab the three random doubles that determine the color
				glColor4f(randColorVect.at(i), randColorVect.at(i+1), randColorVect.at(i+2), 0.5);
			}
			
			//set three points as ponits of the triangle
